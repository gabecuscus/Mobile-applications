...
//https://new.miami.edu/feeds/latest-25.xml
...
class _MyUmAppState extends State<MyUmApp> {
  ...
  void initState() {
    ...
    fetchXML('https://news.miami.edu/feeds/latest-25.xml').then((data) {
      // data was received
      // print(data)
      final documnet = XmlDocument.parse(data);
      final newsitem = documnet.findAllElements('newsitem');
      ...
      newsitem.forEach((newsitem) {
        ...
        // to add items to a list or a collection, we use the .add method
        myNews.add(CustomeCard(img!, title!, summary!));
      });
    }).catchError((error) {
    ...
    fetchXML('https://news.miami.edu/frost/feeds/all-news-15.xml').then((data) {
      ...
      newsitem.forEach((newsitem) {
        ...
        // to add items to a list or a collection, we use the .add method
        myNews2.add(CustomeCard(img!, title!, summary!));
      });
    }).catchError((error) {
      print('error getting data: $error');
    });
    ...
    fetchXML('https://news.miami.edu/sonhs/feeds/student-stories-feed.xml').then((data) {
      ...
      newsitem.forEach((newsitem) {
        ...
        // to add items to a list or a collection, we use the .add method
        myNews3.add(CustomeCard(img!, title!, summary!));
      });
    }).catchError((error) {
      print('error getting data: $error');
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        ...
        Container(
          decoration:
              BoxDecoration(color: const Color.fromARGB(255, 238, 241, 247)),
          child: ListView(
            children: <Widget>[
              for (int i = 0; i < 6; i++)
        ...
        Container(
            decoration:
                BoxDecoration(color: Color.fromARGB(255, 238, 241, 247)),
            child: ListView(
              children: <Widget>[
                for (int i = 0; i < 6; i++)
      ...
      bottomNavigationBar: NavigationBar(
          ...
          destinations: <Widget>[
            ...
            NavigationDestination(
                icon: Icon(Icons.accessibility_new_sharp), label: "hands up"),
            NavigationDestination(
                icon: Icon(Icons.accessibility_rounded), label: "hands down")
          ]),
    );
  }
}
