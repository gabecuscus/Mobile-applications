...
export default function ExpandingThomasAttractorScene() {
  const { scrollYProgress } = useScroll()
  const [scrollPosition, setScrollPosition] = useState(0)

  // Track scroll position for expanding effect
  useEffect(() => {
    return scrollYProgress.onChange((latest) => {
  ...
  const containerStyle = {
    ...
    height: "100%",
    zIndex: -1,
    opacity: Math.min(0.7, 0.3 + scrollPosition * 0.4), // Increase opacity as we scroll
    pointerEvents: "none", // Allow clicking through to content
  } as React.CSSProperties

  // Calculate camera position based on scroll
  // As we scroll down, the camera moves back to show more of the scene
  const cameraPosition = [0, 0, 8 + scrollPosition * 10]

  return (
    <>
      {/* Hero section canvas - visible at the top */}
      <div className="w-full h-[400px] md:h-[500px] rounded-lg overflow-hidden relative z-10 mb-10">
      ...
      {/* Background canvas - expands as we scroll */}
      <div style={containerStyle}>
        <Canvas camera={{ position: cameraPosition, fov: 50 + scrollPosition * 20 }}>
          <ambientLight intensity={0.5} />
          <pointLight position={[10, 10, 10]} intensity={1} />
          <InstancedRectangles count={250} />
        </Canvas>
      </div>
    </>
  )
